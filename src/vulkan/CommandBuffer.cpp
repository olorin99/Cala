...
cala::vk::CommandBuffer::CommandBuffer(Device& device, VkQueue queue, VkCommandBuffer buffer)
    : _device(&device),
    _buffer(buffer),
    _queue(queue),
    _active(false),
...
cala::vk::CommandBuffer::~CommandBuffer() {}

cala::vk::CommandBuffer::CommandBuffer(CommandBuffer &&rhs) noexcept
    : _device(nullptr),
...
cala::vk::CommandBuffer &cala::vk::CommandBuffer::operator=(CommandBuffer &&rhs) noexcept {
    std::swap(_device, rhs._device);
    std::swap(_buffer, rhs._buffer);
    std::swap(_queue, rhs._queue);
    std::swap(_active, rhs._active);
...
bool cala::vk::CommandBuffer::begin() {
...
bool cala::vk::CommandBuffer::end() {
    if (!_active)
        return false;
...
void cala::vk::CommandBuffer::begin(RenderPass &renderPass, VkFramebuffer framebuffer, std::pair<u32, u32> extent) {
...
void cala::vk::CommandBuffer::begin(Framebuffer &framebuffer) {
    begin(framebuffer.renderPass(), framebuffer.framebuffer(), framebuffer.extent());
    _pipelineKey.framebuffer = &framebuffer;
}

void cala::vk::CommandBuffer::end(RenderPass &renderPass) {
    vkCmdEndRenderPass(_buffer);
}

void cala::vk::CommandBuffer::end(Framebuffer &framebuffer) {
    end(framebuffer.renderPass());
}


void cala::vk::CommandBuffer::bindProgram(const ShaderProgram& program) {
...
void cala::vk::CommandBuffer::bindAttributes(std::span<Attribute> attributes) {
...
void cala::vk::CommandBuffer::bindBindings(std::span<VkVertexInputBindingDescription> bindings) {
...
void cala::vk::CommandBuffer::bindAttributeDescriptions(std::span<VkVertexInputAttributeDescription> attributes) {
...
void cala::vk::CommandBuffer::bindVertexArray(std::span<VkVertexInputBindingDescription> bindings,
                                                           std::span<VkVertexInputAttributeDescription> attributes) {
    bindBindings(bindings);
    bindAttributeDescriptions(attributes);
}

void cala::vk::CommandBuffer::bindViewPort(const ViewPort &viewport) {
...
void cala::vk::CommandBuffer::bindRasterState(RasterState state) {
...
void cala::vk::CommandBuffer::bindDepthState(DepthState state) {
...
void cala::vk::CommandBuffer::bindBlendState(BlendState state) {
...
void cala::vk::CommandBuffer::bindPipeline() {
...
void cala::vk::CommandBuffer::bindBuffer(u32 set, u32 binding, BufferHandle buffer, u32 offset, u32 range, bool storage) {
    assert(buffer);
...
void cala::vk::CommandBuffer::bindBuffer(u32 set, u32 binding, BufferHandle buffer, bool storage) {
    assert(buffer);
    assert(set < MAX_SET_COUNT && "set is greater than valid number of descriptor sets");
...
void cala::vk::CommandBuffer::bindImage(u32 set, u32 binding, Image::View& image, SamplerHandle sampler) {
...
void cala::vk::CommandBuffer::bindImage(u32 set, u32 binding, cala::vk::ImageHandle image, cala::vk::SamplerHandle sampler) {
...
void cala::vk::CommandBuffer::bindDescriptors() {
...
void cala::vk::CommandBuffer::clearDescriptors() {
    for (auto& setKey : _descriptorKey)
        setKey = DescriptorKey{};
}


void cala::vk::CommandBuffer::bindVertexBuffer(u32 first, BufferHandle buffer, u32 offset) {
    assert(buffer);
...
void cala::vk::CommandBuffer::bindVertexBuffers(u32 first, std::span<VkBuffer> buffers, std::span<VkDeviceSize> offsets) {
    assert(buffers.size() == offsets.size());
    vkCmdBindVertexBuffers(_buffer, first, buffers.size(), buffers.data(), offsets.data());
    _indexBuffer = {};
}

//void cala::vk::CommandBuffer::bindVertexBuffer(u32 first, Buffer &buffer, u32 offset) {
//    bindVertexBuffer(first, buffer.buffer(), offset);
//}

void cala::vk::CommandBuffer::bindIndexBuffer(BufferHandle buffer, u32 offset) {
    assert(buffer);
...
void cala::vk::CommandBuffer::draw(u32 count, u32 instanceCount, u32 first, u32 firstInstance, bool indexed) {
...
void cala::vk::CommandBuffer::drawMeshTasks(u32 x, u32 y, u32 z) {
    assert(_boundProgram);
    auto localSize = _boundProgram->localSize();
    drawMeshTasksWorkGroups(std::ceil(x / static_cast<f32>(localSize.x())), std::ceil(y / static_cast<f32>(localSize.y())), std::ceil(z / static_cast<f32>(localSize.z())));
}
...
void cala::vk::CommandBuffer::drawMeshTasksIndirectCount(cala::vk::BufferHandle buffer, u32 bufferOffset, cala::vk::BufferHandle countBuffer, u32 countOffset, u32 stride) {
    assert(!_pipelineDirty);
    assert(!_descriptorDirty);
    assert(countBuffer->size() > countOffset);
    u32 maxDrawCount = (buffer->size() - bufferOffset) / stride;
    ...
    ++_drawCallCount;
    writeBufferMarker(PipelineStage::TASK_SHADER, "vkCmdDrawMeshTasksIndirectCountEXT::TASK");
    writeBufferMarker(PipelineStage::MESH_SHADER, "vkCmdDrawMeshTasksIndirectCountEXT::MESH");
    writeBufferMarker(PipelineStage::FRAGMENT_SHADER, "vkCmdDrawMeshTasksIndirectCountEXT::FRAGMENT");
}
...
void cala::vk::CommandBuffer::clearBuffer(cala::vk::BufferHandle buffer, u32 clearValue) {
    vkCmdFillBuffer(_buffer, buffer->buffer(), 0, buffer->size(), clearValue);
}

void cala::vk::CommandBuffer::pipelineBarrier(std::span<VkBufferMemoryBarrier2> bufferBarriers, std::span<VkImageMemoryBarrier2> imageBarriers) {
...
void cala::vk::CommandBuffer::pipelineBarrier(std::span<Image::Barrier> imageBarriers) {
...
void cala::vk::CommandBuffer::pipelineBarrier(std::span<Buffer::Barrier> bufferBarriers) {
...
void cala::vk::CommandBuffer::pipelineBarrier(std::span<MemoryBarrier> memoryBarriers) {
...
void cala::vk::CommandBuffer::pushDebugLabel(std::string_view label, std::array<f32, 4> colour) {
#ifndef NDEBUG
...
void cala::vk::CommandBuffer::popDebugLabel() {
#ifndef NDEBUG
...
void cala::vk::CommandBuffer::startPipelineStatistics() {
...
void cala::vk::CommandBuffer::stopPipelineStatistics() {
...
std::expected<bool, cala::vk::Error> cala::vk::CommandBuffer::submit(std::span<SemaphoreSubmit> waitSemaphores, std::span<SemaphoreSubmit> signalSemaphores, VkFence fence) {
...
bool cala::vk::CommandBuffer::PipelineEqual::operator()(const PipelineKey &lhs, const PipelineKey &rhs) const {
    return 0 == memcmp((const void*)&lhs, (const void*)&rhs, sizeof(lhs));
}

void cala::vk::CommandBuffer::writeBufferMarker(cala::vk::PipelineStage stage, std::string_view cmd) {
#ifndef NDEBUG
