...
cala::vk::RenderPass::RenderPass(Device& driver, std::span<Attachment> attachments)
...
cala::vk::RenderPass::~RenderPass() {
    if (_renderPass != VK_NULL_HANDLE)
        vkDestroyRenderPass(_device, _renderPass, nullptr);
}

cala::vk::RenderPass::RenderPass(RenderPass &&rhs) noexcept
...
cala::vk::RenderPass &cala::vk::RenderPass::operator==(RenderPass &&rhs) noexcept {
    if (this == &rhs)
        return *this;
    std::swap(_device, rhs._device);
    std::swap(_renderPass, rhs._renderPass);
...
VkFramebuffer cala::vk::RenderPass::framebuffer(std::span<VkImageView> attachments, u32 width, u32 height) {
